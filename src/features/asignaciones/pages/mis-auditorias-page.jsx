...
const getPeriodLabel = (c) => {
  if (!c) return '';
  const label = c.numeroCorte === 1 ? 'Primer periodo' : c.numeroCorte === 2 ? 'Segundo periodo' : `Periodo ${c.numeroCorte}`;
      ...
      if (key && key.startsWith(prefix)) {
        ...
        if (item) {
          ...
          const respondidas = Object.values(respuestas).filter(
            r => r.opcionFormularioIds?.length > 0 || r.hallazgo?.trim()
          ).length;
...
function CompartirAuditoriaModal({ asignacion, isOpen, onClose, onInvitacionChange }) {
  const [loading, setLoading] = useState(false);
  const [revocando, setRevocando] = useState(false);
  const [enlace, setEnlace] = useState(null);
  const [url, setUrl] = useState('');
  const [error, setError] = useState('');
  const areaNombre = asignacion?.objetivoAuditoria?.area?.nombre ?? asignacion?.objetivoAuditoria?.nombreAreaSnapshot ?? 'Área';
  const ciclo = asignacion?.objetivoAuditoria?.cicloAuditoria;

  useEffect(() => {
    if (!isOpen || !asignacion) return undefined;
    let active = true;
    setLoading(true);
    setError('');
    setEnlace(null);
    ...
    if (!getPublicAppBaseUrl()) {
      setError('Configura VITE_PUBLIC_APP_URL con una URL pública. No se genera enlace con localhost.');
      setLoading(false);
      return undefined;
    }

    asignacionesApi.crearInvitacion(asignacion.id)
      .then((response) => {
        if (!active) return;
        const invitacionUrl = buildPublicAppUrl(`/invitado/${response.token}`);
        setEnlace(response.enlace);
        setUrl(invitacionUrl);
        onInvitacionChange(asignacion.id, response.enlace);
      })
      .catch((err) => {
        if (active) setError(err?.message || 'No se pudo crear la invitación.');
      })
      .finally(() => {
  ...
  }, [asignacion, isOpen, onInvitacionChange]);
  ...
  const revocar = async () => {
    setRevocando(true);
    try {
      await asignacionesApi.revocarInvitacionActiva(asignacion.id);
      onInvitacionChange(asignacion.id, null);
      notify.success('Invitación revocada.');
      onClose();
    } catch (err) {
      notify.error(err?.message || 'No se pudo revocar la invitación.');
    } finally {
      setRevocando(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="sm">
      <ModalHeader title="Compartir auditoría" onClose={onClose} />
      <ModalBody className="space-y-5 text-center">
        <div>
          <h3 className="text-2xl font-black uppercase text-slate-950">{areaNombre}</h3>
          <p className="mt-1 text-sm font-bold text-slate-500">{ciclo ? getPeriodLabel(ciclo) : 'Evaluación 5S'}</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3">
            <QrCode value={url} label="QR de invitación" />
            <p className="max-w-full truncate rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500">
              {url}
            </p>
          </div>
        )}
      ...
      <ModalFooter>
        {enlace && (
          <Button type="button" variant="danger" icon="link_off" onClick={revocar} isLoading={revocando}>
            Revocar invitación
          </Button>
        )}
        <Button type="button" variant="outline" icon="content_copy" onClick={copiar} disabled={!url || loading}>
          Copiar enlace
        </Button>
        <Button type="button" icon="ios_share" onClick={compartir} disabled={!url || loading}>
          Compartir
        </Button>
      </ModalFooter>
    </Modal>
...
export function MisAuditoriasPage() {
  ...
  const canExecuteAudit =
    AUDIT_EXECUTION_ROLES.includes(user?.rol);

  const [executables, setExecutables] = useState([]);
  const [loading, setLoading] = useState(true);
  const [asignacionCompartir, setAsignacionCompartir] = useState(null);
  ...
  useEffect(() => {
    ...
    async function fetchExecutables() {
      ...
      try {
        ...
        const list = Array.isArray(res?.datos)
          ? res.datos
          : Array.isArray(res)
            ? res
            : [];
  ...
  const activeCiclo = useMemo(() => {
    return (
      executables[0]?.objetivoAuditoria?.cicloAuditoria ??
      null
    );
  }, [executables]);

  const totalPeriodo = executables.length;

  const completadasPeriodo = useMemo(() => {
    return executables.filter(
      (asig) => asig.estado === 'COMPLETADA',
    ).length;
  }, [executables]);

  const pendientesPeriodo =
    totalPeriodo - completadasPeriodo;

  const activePeriodUrgency = useMemo(() => {
    return (
      executables.find(
        (asig) => asig.estado !== 'COMPLETADA',
      )?.infoPeriodo ?? null
  ...
  const realizadasPeriodoLst = useMemo(() => {
    return executables.filter(
      (asig) => asig.estado === 'COMPLETADA',
    );
  }, [executables]);
  ...
  const actualizarInvitacionLocal = useCallback(
    (asignacionId, invitacionActiva) => {
      setExecutables((actuales) =>
        actuales.map((asig) =>
            ...
            >
              <div>
                ...
                <h2 className="mt-0.5 text-xl font-black text-slate-950">
                  {getPeriodLabel(activeCiclo).toUpperCase()}
                </h2>
              ...
              >
                <span className="text-sm font-bold text-slate-700">
                  {completadasPeriodo} realizadas ·{' '}
                  {pendientesPeriodo} pendientes
                </span>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${
                        totalPeriodo > 0
                          ? (completadasPeriodo / totalPeriodo) * 100
                          : 0
                      }%`,
                    }}
                  />
                ...
                {activePeriodUrgency && (
                  <span
                    className={`mt-1 text-xs font-black ${
                      activePeriodUrgency.color === 'rojo'
                        ? 'text-red-600'
                        : activePeriodUrgency.color === 'ambar'
                          ? 'text-amber-600'
                          : 'text-emerald-700'
                    }`}
                  >
            ...
            <CardBody className="flex flex-col items-center gap-4 py-12 text-center">
              ...
              <div>
                ...
                <p className="mt-2 max-w-md text-sm font-semibold leading-6 text-slate-600">
                  Terminaste todas tus auditorías del{' '}
                  {activeCiclo
                    ? getPeriodLabel(activeCiclo)
                    : 'periodo actual'}
                  .
                </p>

                <p className="mt-1 text-sm font-black text-emerald-800">
                  {completadasPeriodo} de {totalPeriodo} realizadas
                </p>
              </div>
            </CardBody>
          </Card>
        ...
        {(porRealizarList.length > 0 ||
          ...
          <>
            ...
            <div className="block space-y-3 md:hidden">
              ...
              {porRealizarList.map((asig) => {
                const areaNombre =
                  asig.objetivoAuditoria?.area?.nombre ??
                  asig.objetivoAuditoria?.nombreAreaSnapshot ??
                  'Área';
                ...
                const compartida = Boolean(
                  asig.invitacionActiva,
                );

                const progresoLabel = enCurso
                  ? borrador.total > 0
              ...
              {realizadasPeriodoLst.map((asig) => {
                const areaNombre =
                  asig.objetivoAuditoria?.area?.nombre ??
                  asig.objetivoAuditoria
                    ?.nombreAreaSnapshot ??
                  'Área';
